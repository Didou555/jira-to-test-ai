...
export const UserManagementSection = () => {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("delete-user", {
        body: { userId: showDeleteConfirm.user_id },
      });
      if (error) throw new Error(error.message);
      if (data?.error) throw new Error(data.error);
  ...
  const openApiKeys = async (userId: string) => {
    ...
    try {
      ...
      if (data) {
        setApiKeys({
          jira_base_url: data.jira_base_url || "", jira_email: data.jira_email || "",
          jira_api_token: data.jira_api_token || "", qmetry_api_token: data.qmetry_api_token || "",
          aws_access_key_id: data.aws_access_key_id || "", aws_secret_access_key: data.aws_secret_access_key || "",
          aws_region: data.aws_region || "us-east-1", aws_session_token: data.aws_session_token || "",
        });
      } else {
        setApiKeys({ jira_base_url: "", jira_email: "", jira_api_token: "", qmetry_api_token: "", aws_access_key_id: "", aws_secret_access_key: "", aws_region: "us-east-1", aws_session_token: "" });
      }
    } catch (e: any) {
      toast({ title: t.error, description: e.message, variant: "destructive" });
    }
  ...
  const handleSaveApiKeys = async () => {
    ...
    try {
      const payload = {
        ...
        jira_base_url: apiKeys.jira_base_url || null, jira_email: apiKeys.jira_email || null,
        jira_api_token: apiKeys.jira_api_token || null, qmetry_api_token: apiKeys.qmetry_api_token || null,
  ...
  const SecretField = ({ label, field, placeholder }: { label: string; field: keyof typeof apiKeys; placeholder: string }) => (
    <div className="space-y-1">
      ...
      <div className="relative">
        <Input type={showSecrets[field] ? "text" : "password"} value={apiKeys[field]} onChange={(e) => setApiKeys((prev) => ({ ...prev, [field]: e.target.value }))} placeholder={placeholder} />
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t.deleteConfirmDesc}
              <br />
              <strong className="mt-2 block">{showDeleteConfirm?.display_name || showDeleteConfirm?.username}</strong>
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={!!showApiKeys} onOpenChange={() => setShowApiKeys(null)}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{t.apiKeysDesc}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowApiKeys(null)}>{t.cancel}</Button>
